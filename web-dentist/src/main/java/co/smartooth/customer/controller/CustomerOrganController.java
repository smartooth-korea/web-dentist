...
public class CustomerOrganController {
	 ...
	 * 수정일 : 2023. 08. 22
	 */
	@PostMapping(value = {"/customer/organ/selectDentalHospitalVisitList.do"})
	@ResponseBody
	public HashMap<String,Object> selectDentalHospitalVisitList(HttpServletRequest request, @RequestBody HashMap<String, Object> paramMap) {
       
	    
		Logger logger = LoggerFactory.getLogger(getClass());

	    
	    logger.debug("========== Customer.OrganController ========== /customer/organ/selectDentalHospitalVisitList.do ==========");
	    logger.debug("========== Customer.OrganController ========== /customer/organ/selectDentalHospitalVisitList.do ==========");
	    logger.debug("========== Customer.OrganController ========== /customer/organ/selectDentalHospitalVisitList.do ==========");
	    logger.debug("========== Customer.OrganController ========== /customer/organ/selectDentalHospitalVisitList.do ==========");
		...
		HashMap<String,Object> hm = new HashMap<String,Object>();
		List<HashMap<String, Object>> selectDentalHospitalVisitList = new ArrayList<HashMap<String, Object>>();
		...
		// 종료날짜 (오늘)
		measureEndDt = now.format(formatter);
...
//		if(tokenValidation) {
			try {
				// 개인정보 동의한 치과 목록
				selectDentalHospitalVisitList = customerOrganService.selectDentalHospitalVisitList(userId, measureStartDt, measureEndDt);
			} catch (Exception e) {
				hm.put("code", "500");
				//if(lang.equals("ko")) {
				hm.put("msg", "방문한 치과 목록을 조회에 실패했습니다.\n관리자에게 문의해주시기 바랍니다.");
				//}else if(lang.equals("en")) {
				//	hm.put("msg", "Server Error");
				//}
				e.printStackTrace();
		...
		hm.put("userAuthToken", userAuthToken);
		hm.put("selectDentalHospitalVisitList", selectDentalHospitalVisitList);
		// 메시지 RETURN
		hm.put("code", "000");
		//if(lang.equals("ko")) {
		hm.put("msg", "조회 성공");
